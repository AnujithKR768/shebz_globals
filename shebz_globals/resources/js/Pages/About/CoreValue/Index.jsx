...
export default function Index({ auth, items }) {
    const del = (id) => {
        if (confirm("Delete this Core Value?")) {
            router.delete(route("corevalue.admin.destroy", id));
        }
    };

    return (
        <AuthenticatedLayout user={auth.user} header="Core Values">
            <Head title="Core Values" />

            <div className="max-w-7xl mx-auto bg-white shadow rounded-md overflow-hidden border">
                <div className="flex justify-between items-center p-4">
                    <h1 className="text-xl font-bold text-gray-800">Core Values List</h1>

                    <Link
                        href={route("corevalue.admin.create")}
                        ...
                        <tbody>
                            {items.data.map((item, index) => (
                                <tr key={item.id} className="border-b hover:bg-gray-50 text-sm">
                                    ...
                                    <td className="p-3 font-semibold">{item.value_title}</td>
                                    ...
                                    <td className="p-3 text-center">
                                        <Link
                                            href={route("corevalue.admin.edit", item.id)}

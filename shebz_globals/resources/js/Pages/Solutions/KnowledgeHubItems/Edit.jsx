...
export default function Edit({ auth, item, hubList = [] }) {
    const hasHubs = hubList && hubList.length > 0;

    const { data, setData, post, processing, errors } = useForm({
        knowledge_hub_id: item?.knowledge_hub_id || (hasHubs ? hubList[0].id : ""),
        text: item?.text || "",
    ...
    const submit = (e) => {
        ...
        post(route("knowledgehubitem.admin.update", item.id), {
            preserveScroll: true,
        });
    };

    return (
        <AuthenticatedLayout user={auth.user} header="Edit Knowledge Hub Item">
            <Head title="Edit Knowledge Hub Item" />

            <div className="max-w-4xl mx-auto bg-white shadow rounded-md border p-6">

                {/* TITLE */}
                <h1 className="text-2xl font-bold text-gray-800 mb-6">
                    Edit Knowledge Hub Item
                </h1>

                {/* WARNING IF NO HUBS */}
                {!hasHubs && (
                    <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">
                        ⚠️ No Knowledge Hubs found. Please create one first.
                    </div>
                )}

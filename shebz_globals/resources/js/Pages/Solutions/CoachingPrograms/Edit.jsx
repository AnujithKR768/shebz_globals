...
export default function Edit({ auth, item, coachingList }) {
    const { data, setData, post, processing, errors } = useForm({
        coaching_mentorship_id: item.coaching_mentorship_id || "",
        text: item.text || "",
    ...
    const submit = (e) => {
        ...
        post(route("coachingprogram.admin.update", item.id));
    };

    return (
        <AuthenticatedLayout user={auth.user} header="Edit Coaching Program">
            <Head title="Edit Coaching Program" />

            <div className="max-w-4xl mx-auto bg-white shadow rounded-md border p-6">
                <h1 className="text-2xl font-bold text-gray-800 mb-6">
                    Edit Program Include Item
                </h1>
                ...
                <form onSubmit={submit} className="space-y-5">
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                setData("coaching_mentorship_id", e.target.value)
                            }
